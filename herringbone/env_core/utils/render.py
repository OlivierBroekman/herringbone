...
class Render:
    ...
    def fancy_board(board: Board, agent_state: State, property_func=lambda s: s.get_character(), t: int = 0) -> str:
        ...
        rendering_states = [[board.agent if s == agent_state else s for s in row] for row in
                            board.states] if agent_state else board.states

        len_char = max(max(len(str(property_func(s)) or '') for row in board.states for s in row),
                       len(board.agent.get_character()))
    ...
    def preview_Q(mdp: MDP, learned_Q: dict[State, dict[Action, float]]) -> None:
        """Renders a Q-function"""

        states_2d = mdp.get_board().states
        actions = mdp.get_actions()
        cell_width = len(
            f"{max(actions, key=lambda a: len(a.get_character()))}: {max(abs(learned_Q[s][a]) for r in states_2d for s in r for a in actions):.2f}") + 3
